...
acct = get_additional_account(mnemonic, 0)
...
dec_key = coincurve.PrivateKey()
enc_key = dec_key.public_key

tx_hash = game.functions.broadcastEncryptionKey(enc_key.format()).transact()
tx_receipt = web3.eth.wait_for_transaction_receipt(tx_hash)
assert tx_receipt['status']

event_filter = game.events.CommittedMovesReady.create_filter(fromBlock=0)

while True:
    for event in event_filter.get_new_entries():
        for playerId in range(2):
            R, ciphertext = game.functions.committedMoves(playerId).call()
            k = dec_key.ecdh(R)
            nonce = [0]*8
            cipher = ChaCha20.new(key=k, nonce=nonce)
            moves = cipher.decrypt(ciphertext)
            tx_hash = game.functions.revealMoves(playerId, [int(m) for m in moves]).transact()
            tx_receipt = web3.eth.wait_for_transaction_receipt(tx_hash)
            assert tx_receipt['status']
