...
PUBLIC_HOST = os.getenv("PUBLIC_HOST", "http://127.0.0.1:12015")
...
class Launcher(TextIORequestHandler, abc.ABC):
    ...
    def deploy_challenge(self):
        ...
        tx = self.fund(get_player_account(self.mnemonic).address)
        self.web3.eth.wait_for_transaction_receipt(tx)

        challenge_addr = self.deploy()

        os.environ["MNEMONIC"] = self.mnemonic
        os.environ["RPC_URL"] = f"http://127.0.0.1:12015/{self.token}"

        proc = subprocess.Popen(["python3", "challenge/operator_script.py", challenge_addr])
