...
module.exports = {
	async execute(interaction) {
		if(!checkRole(interaction, "incr")){
			return await interaction.reply({content: `We can only do this to a nice expressive face, or a hand in mind`, 
				ephemeral: true});
		}
        const state = await State.findOne({ where: { name: interaction.user.username } });
		if (state){
			old_state = state.state_0
			new_state = ((old_state + 1) % n)
