...
def run_docker_container_with_timeout(timeout_seconds):
    ...
    #proc = Popen(["docker", "run", "--rm", "--security-opt seccomp=unconfined", "-i", "-u", "ctf", "misc-aflmm-level1-chall", "/app/run"])
    proc = Popen(["docker", "run", "--rm", "-i", "-u", "ctf", "misc-aflmm-level1-chall", "/app/run"])
